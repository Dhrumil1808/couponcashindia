...
function exit_id_details()
{
	...
	$sql=$wpdb->prepare('INSERT INTO `exit_id_table` (`id`, `exit_id`, `user_id`,`retailer`,`url`,`cashback`)
     VALUES ("","'.$rand_num.'","'.$current_user->ID.'","'.$_POST['store'].'","'.$_POST['affurl'].'","")');	
      $r=$wpdb->query($sql);
	...
	//exit;
       ...
       {
        if($_POST['store']=="Amazon")
        {
          echo $_POST['affurl'].'&ascsubtag='.$rand_num;  
         exit;  
        }
        else
        {
        echo $_POST['affurl'].'&ascsub='.$rand_num;  
        exit;
       }
     }
   		else{
...
function password()
{
    ...
    echo $mail_check;
        if($mail_check==1) {
            ...
            $message = "Dear $username,<br />We understand you'd like to change your password. Just <a href=".site_url()."/forgot_password?e=".base64_encode($Email).">click here</a> and follow the prompts. And don't forget your password is case sensitive.";
            $from = "coupon@couponcashindia.com";
