...
function twentythirteen_setup() {
	...
	register_nav_menu( 
		...
		'top', __( 'Top Menu', 'twentythirteen' )
	 );
	/*
	 * This theme uses a custom image size for featured images, diplsayed on
...
function set_newuser_cookie() {
	global $wpdb;
   	...
   	if(is_user_logged_in())
   	{
   		...
   		//$insert="INSERT INTO `wp_tracking`(`ID`, `Email`, `exit_ID`) VALUES (NULL,'$email','0')";
			//echo $insert;
			//$wpdb->query($insert);
   		setcookie('sitename_newvisitor',$email,time()+3600*24,"/",".couponcashindia.com");
   		if(isset($_COOKIE['sitename_newvisitor']))
   		{
   			//echo "YEs,cookie is set";
   		}
   	...
   	else
   	{
   			...
   			unset( $_COOKIE['sitename_newvisitor'] );
			setcookie( 'sitename_newvisitor', '', time() - ( 60 * 60 * 24 ) );
			if(isset($_COOKIE['sitename_newvisitor']))
			{
				//echo "Cookie Not destroyed";
			}
			else
			{
...
if( is_user_logged_in() ) { 
	...
	//setcookie('email', 1, time()+3600*24*100, COOKIEPATH, COOKIE_DOMAIN);
} else { 
	$args['menu'] = 'Top menu';
} 
	return $args;
