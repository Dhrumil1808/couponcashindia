jQuery( document ).ready(function() {
  ...
  if (email2 == '') {
    ...
    success: function(response){
        if(response==0)
        {
          jQuery("#error_msg2").text("Email is not registered");
        }
        else if(response==2)
        {
          jQuery("#error_msg2").text("Email is not sent");
        }
        else 
        {
      ...
      jQuery(".note2").show();
      jQuery("#error_msg2").text("Email has benn sent to the registered Email ID");
        }
    } 
      }); 
  }
   ...
   jQuery("#mega-menu-item-1332").click(function (){
       jQuery("#forgotpass-block").dialog({ 
        width: 150,
        height: 310,
        modal: true,
     ...
     jQuery.ajax({
      ...
      cache: false,
          data :{ action:'exit_id_details',affurl:affurl,store:store},
           beforeSend: function(){
                  //alert('#loading_' + value);
                 jQuery('#loading_' + value).show();
              },
          success:function(response) {
            jQuery('#loading_' + value).hide();   
             if(response==0)
            {
